...
//			$html = file_get_contents($url, false, $context);
            ...
            if ($contentType === 'application/pdf') {
                return ['error' => 'incorrect_content_type'];
            }
		...
		try {
			...
			if ($article->getCleanedArticleText() == null) {
				...
				foreach ($candidateNodes as $node) {
					...
					$this->log_debug("Element:\t" . $node->tag->name() . "\tTotal WC:\t" . $this_element_wc . "\tTotal White:\t" . $this_element_whitecount . "\tRatio:\t" . number_format($this_element_wc_ratio,
							2) . "\tElement WC:\t" . $this_element_wc_contribution . "\tChildren WC:\t" . $children_wc . "\tChild Contributors:\t" . $children_num . "\tBest WC:\t" . $best_element_wc . "\tBest Ratio:\t" . number_format($best_element_wc_ratio,
		...
		if ($element->getParent() != null) {
			...
			foreach ($parent->getChildren() as $child) {
				...
				if ($child_wc != 0) {
					...
					$this->log_debug("	  Child: " . $child->tag->name() . " (" . $child->getAttribute('class') . ") WC: " . $child_wc . " Ratio: " . number_format($child_wc_ratio,
							2));
